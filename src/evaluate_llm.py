...
def evaluate_model(prompt_uri, dataset_path, intent_column='intent', command_column='command', 
    ...
    with mlflow.start_run():   
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_example = {
                executor.submit(process_example, row): row 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Evaluate bash command recovery prompt from MLflow against a dataset')
    parser.add_argument('--prompt-uri', type=str, required=True, help='MLflow prompt URI (e.g., "prompts:/generate-bash-description/2")')
    parser.add_argument('--dataset', type=str, required=True, help='Path to input CSV dataset')
    parser.add_argument('--intent-column', type=str, default='description', help='Name of the intent/description column')
    parser.add_argument('--command-column', type=str, default='command', help='Name of the command column')
    parser.add_argument('--max-workers', type=int, default=5, help='Number of parallel workers')
    parser.add_argument('--model', type=str, required=False, default="local-model", help='Model name')
