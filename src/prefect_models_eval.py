...
@task(description= "calculates bleu2 and bleu4", cache_policy=NO_CACHE)
def calculate_bleu_scores(generated, reference):
    ...
    bleu2 = sentence_bleu([ref_tokens], gen_tokens, weights=(0.5, 0.5), 
...
def evaluate_model(prompt_uri, dataset_path, intent_column='intent', command_column='command', 
    ...
    with mlflow.start_run():   
        try:
            ...
            logger.info(f"Successfully loaded prompt from: {prompt_uri}")
        except Exception as e:
            logger.error(f"Failed to load prompt from {prompt_uri}: {e}")
            return
        ...
        try:
            ...
            if intent_column not in df.columns or command_column not in df.columns:
                raise ValueError(f"Dataset must contain '{intent_column}' and '{command_column}' columns")
        ...
        @task(description="process one command", cache_policy=NO_CACHE)
        def process_example(row):
            ...
            try:
                ...
                bleu2, bleu4 = calculate_bleu_scores(generated_command, reference_command)
                ...
                logger.error(f"Error processing intent '{intent[:50]}...': {e}")
        ...
        future_to_example = [
            process_example.with_options(name=f"process-command-{i}").submit(row)
            for i, row in df.iterrows()
        ]
        
        for i, future in enumerate(as_completed(future_to_example)):
            logger.info(f"process {i} command")
            try:
        ...
        if results:
            ...
            exact_match_rate = exact_matches / len(successful_results) if successful_results else 0
            avg_bleu2 = sum(r["bleu2"] for r in successful_results) / len(successful_results) if successful_results else 0
            avg_bleu4 = sum(r["bleu4"] for r in successful_results) / len(successful_results) if successful_results else 0
            ...
            mlflow.log_metric("successful_processing_rate", len(successful_results) / len(results))
            
            logger.info(f"Evaluation Results:")
            logger.info(f"Exact Match Rate: {exact_match_rate:.4f}")
            logger.info(f"Average BLEU-2: {avg_bleu2:.4f}")
            logger.info(f"Average BLEU-4: {avg_bleu4:.4f}")
            logger.info(f"Successful Processing: {len(successful_results)}/{len(results)}")
...
if __name__ == "__main__":
    ...
    evaluate_model.serve(parameters={
        "dataset_path":"data/test.csv",
        "intent_column":"description",
