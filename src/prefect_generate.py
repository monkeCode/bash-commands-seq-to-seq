...
def process_single_command_internal(command, server_pool):
    ...
    try:
        ...
        prompt = mlflow.genai.load_prompt(
            "prompts:/generate-bash-description/12"
        ).template
...
def process_command_batch(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_command = {
            executor.submit(
                process_single_command_internal, command, server_pool
            ): command
    ...
    logger.info(
        f"📊 Батч {batch_number} завершен. Обработано: {processed_so_far}/{total_commands}, осталось: {remaining}"
    )
        ...
        try:
            with open(output_csv, "a", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["command", "reasoning", "description", "is_command"]
                )
                if f.tell() == 0:
                    writer.writeheader()
...
def command_description_flow(
    ...
    """
    print("🚀 Starting command description pipeline")
    ...
    if os.path.exists(output_csv):
        ...
        print(
            f"Found {len(existing_commands)} existing commands, {len(commands)} new to process"
        )
    else:
    ...
    batches = [
        commands[i : i + batch_size] for i in range(0, len(commands), batch_size)
    ]
    ...
    if os.path.exists(output_csv):
        ...
        return {"status": "failed", "error": "No output file created"}
