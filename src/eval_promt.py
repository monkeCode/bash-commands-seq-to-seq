...
def evaluate_prompt(prompt_uri, dataset_path, command_column='command', model="local-model", max_workers=1):
    """
    ...
    with mlflow.start_run():   
        ...
        chain = prompt_template_obj | llm | StrOutputParser() | clean_json_output | parser
        ...
        d = pd.merge(left=results_df, right= df[["command","real_is_command"]], how="left", on="command")
        dataset = mlflow.data.from_pandas(d, predictions= "is_command", targets="real_is_command")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Evaluate a prompt from MLflow against a dataset')
    ...
    parser.add_argument('--model', type=str, required=False, default="local-model", help='model')
