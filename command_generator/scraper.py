...
class WebScraper:
    def __init__(self, utilities=None):
        ...
        self.data = {}
        self.relevant_flags = {}
        self.lock = threading.Lock()  # Для потокобезопасного доступа к данным
    ...
    def _process_utility(self, utility):
        """Обработка одной утилиты в потоке"""
        utility_urls = [
            f'https://man7.org/linux/man-pages/man1/{utility}.1.html',
            f'https://man7.org/linux/man-pages/man1/{utility}.2.html'
        ]
        ...
        for url in utility_urls:
            try:
                ...
                if r.status_code == 200:
                    ...
                    syntax = self._generate_syntax(utility, desc.split('\n')[1].strip())
    ...
    def extract_utilities(self):
        """Многопоточное извлечение информации об утилитах"""
        print(f"Scraping the internet for {len(self.utilities)} utilities...")
...
if __name__ == "__main__":
    ...
    with open("command_generator/all_commands.txt") as f:
        for l in f.readlines():
            c.append(l.replace("\n", ""))
    scraper = WebScraper(c[0:20])
    scraper.extract_utilities()
    #scraper.display_structures()
